...
from apscheduler.triggers.cron import CronTrigger  # Добавленный импорт
    ...
    def handle(self, *args, **options):
        ...
        for ns in NewsletterSettings.objects.filter(is_active=True):
            ...
            if ns.frequency == 'daily':
                ...
                next_run_time = current_time + relativedelta(months=1)
            ...
            scheduler.add_job(
                ...
                id=f"send_letter_{ns.id}",
                max_instances=1,
                replace_existing=True,
                kwargs={'newsletter_settings_id': ns.id}
            )
